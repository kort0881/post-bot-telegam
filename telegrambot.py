...
AI_PRIORITY_KEYWORDS = [
    # Общие термины ИИ (русские)
    "нейросеть", "нейросети", "нейронная сеть", "ии", 
    "искусственный интеллект", "машинное обучение",
    "глубокое обучение", "генеративный",
    # LLM и модели
    "llm", "gpt", "gpt-4", "gpt-5", "chatgpt", "claude", "gemini",
    "copilot", "mistral", "llama", "qwen", "gigachat", "yandexgpt",
    "языковая модель", "большая языковая модель",
    # Компании ИИ
    "openai", "anthropic", "deepmind", "hugging face",
    "stability ai", "сбер", "яндекс",
    # Генеративные модели
    "stable diffusion", "midjourney", "dall-e", "sora", "kandinsky",
    "генерация изображений", "генерация текста", "генерация видео",
    "диффузионная модель",
    # ML термины
    "трансформер", "обучение модели", "дообучение", "файнтюнинг",
    "датасет", "нейросетевой", "мультимодальный",
    # Применения ИИ
    "чат-бот", "голосовой помощник", "алиса", "маруся",
    "автопилот", "распознавание лиц", "распознавание речи",
    "компьютерное зрение", "рекомендательная система",
    # Новые разработки
    "agi", "сильный ии", "агент", "ии-агент", "ии-ассистент",
    "контекстное окно", "токенизация",
    # Бренды
    "сбериск", "yandex cloud", "cloud ml"
]
...
GENERAL_KEYWORDS = [
    ...
    # Железо
    "процессор", "чип", "видеокарта", "gpu", "cpu",
    "смартфон", "ноутбук", "гаджет", "робот", "квантовый",
    # Кибербезопасность (остаточный принцип)
    "уязвимость", "взлом", "хакер", "хакеры",
    "кибератака", "безопасность", "утечка данных",
    "патч", "вредоносный", "фишинг", "мошенники"
]
...
EXCLUDE_KEYWORDS = [
    ...
    "крипто", "bitcoin", "биткоин", "биткойн", "ethereum", "nft", "блокчейн",
...
def load_articles_from_sites() -> List[Dict]:
    """
    Загружает статьи только из русскоязычных источников.
    Приоритет: ИИ/нейросети. Кибербезопасность — остаточный принцип.
    """
    articles: List[Dict] = []

    # === ПРИОРИТЕТ: ИИ и нейросети (русскоязычные) ===

    # Habr — хаб искусственного интеллекта
    ...
    # ServerNews — серверы и технологии
    articles.extend(load_rss(
    ...
    # Anti-Malware — безопасность
    articles.extend(load_rss(
        "https://www.anti-malware.ru/rss/news",
        "Anti-Malware"
    ))
...
def filter_articles(articles: List[Dict]) -> List[Dict]:
    ...
    Сначала возвращает AI-статьи, потом остальные.
    """
    ai_articles = []
    general_articles = []
    ...
    # Возвращаем: сначала AI статьи, потом остальные
...
def short_summary(title: str, summary: str, link: str) -> Optional[str]:
    ...
    prompt = (
        ...
        "- В конце: 2-3 релевантных хештега (#AI #нейросети #технологии).\n"
