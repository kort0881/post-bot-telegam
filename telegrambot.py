...
# Безопасный лимит подписи к медиа в Telegram (~1024 символа, берём с запасом)[web:54][web:48]
TELEGRAM_CAPTION_LIMIT = 1000

# ============ СТИЛИ ПОСТОВ (НОВОСТНЫЕ, НО ПОЖИВЕЕ) ============

POST_STYLES = [
    {
        "name": "news_report",
        "intro": "Ты ведёшь живой техно-новостной канал про ИИ. Показываешь главное, без рекламы и лишнего пафоса.",
        "tone": "Спокойный, но не скучный. Короткие, понятные фразы, немного эмоций через эмодзи.",
        "emojis": "🤖📊📰"
    },
    {
        "name": "explainer",
        "intro": "Ты объясняешь сложные ИИ-истории простым языком. Помогаешь быстро понять, что произошло и зачем.",
        "tone": "Добрый, пояснительный тон. Немного вовлечения, но без продаж и лозунгов.",
        "emojis": "🧠🔍📌"
    },
    {
        "name": "use_case",
        "intro": "Ты показываешь, как новое исследование или модель могут пригодиться в реальных задачах.",
        "tone": "Практичный, живой тон. Чётко: что сделали, чем лучше, где может пригодиться.",
        "emojis": "⚙️✅💡"
    }
]
...
• Только русский язык.
• Используй 2–4 эмодзи из набора {style['emojis']} и общих тех-эмодзи (⚙️, 💻, 📡, 📈, 🛰️) — в начале и/или в конце предложений, чтобы текст выглядел живее.
...
ЗАПРЕЩЕНО:
• Прямые призывы: «попробуйте», «оцените», «не пропустите» и т.п.
...
def decorate_post(text: str, topic: str) -> str:
    """Украшает пост: шапка с эмодзи по теме + лёгкий разделитель перед источником."""
    topic_icon_map = {
        "llm": "🤖",
        "ai": "🧠",
        "image_gen": "🎨",
    ...
    top_line = f"{icon} {icon} {icon}  Обзор по ИИ  {icon} {icon} {icon}"
    separator = "\n\n──────────────────\n\n"  # более аккуратный разделитель
...
def short_summary(title: str, summary: str, link: str) -> Optional[str]:
    ...
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "Ты ведёшь новостной канал про ИИ. Пишешь по фактам, но живо, "
                        "используешь эмодзи для структуры и настроения, без рекламного тона."
                    )
                },
        ...
        # Гарантия, что подпись не перепрыгнет лимит Telegram[web:54][web:48]
