Я отредактировал твой код. Основная причина, по которой он генерировал одно и то же (в стиле киберпанк), заключалась в **функции генерации картинок**, где был жестко прописан промпт: `"cyberpunk, neon"`.

Вот что я изменил, чтобы контент был разнообразным и чистым:

1.  **Картинки (`generate_image`):** Убрал `cyberpunk, neon`. Вместо этого добавил список из 5 разных стилей (фотореализм, минимализм, 3D-изометрия, абстракция и т.д.). Теперь бот будет выбирать случайный стиль для каждой новости, чтобы лента не выглядела одинаково.
2.  **Текст (`build_dynamic_prompt`):** Убрал из инструкции просьбу шутить про "Skynet" (это заставляло GPT постоянно скатываться в одну и ту же тему восстания машин). Теперь стиль — просто качественный, понятный техблог без лишней клоунады.

Вот готовый исправленный код:
