...
BAD_PHRASES = [
    # Призывы к действию
    "стоит попробовать", "попробуйте", "оцените", "обязательно оцените",
    "не упустите", "успейте", "только сейчас", "прямо сейчас",
    "немедленно", "срочно", "скорее", "поспешите",
    
    # Рекламные клише
    "must-have", "must have", "must‑have", "маст-хэв",
    "это меняет правила игры", "меняет правила игры", "game changer",
    "обязателен к использованию", "идеальный инструмент",
    "лучшее решение", "лучший выбор", "лучший инструмент",
    "революционный продукт", "уникальное предложение",
    "убийца", "killer", "взорвёт", "взорвет",
    
    # Оценочные преувеличения
    "невероятный", "потрясающий", "восхитительный", "умопомрачительный",
    "фантастический", "грандиозный", "эпохальный", "исторический",
    "переломный момент", "новая эра", "будущее уже здесь",
    "мир никогда не будет прежним", "изменит всё",
    
    # Манипулятивные фразы
    "вы должны", "вам нужно", "нельзя пропустить", "нельзя игнорировать",
    "каждый должен", "все уже", "пора и вам",
    
    # Восторженный тон
    "вау", "wow", "ого", "ничего себе", "обалдеть",
    "круто", "супер", "класс", "огонь", "бомба",
    
    # Пустые усилители
    "абсолютно", "совершенно", "полностью", "тотально",
    "буквально", "реально", "просто космос",
    
    # Клише про технологии
    "будущее наступило", "мы живём в будущем", "sci-fi стало реальностью",
    "как в фантастике", "мир изменился навсегда"
]

def is_too_promotional(text: str) -> bool:
    """Проверяет текст на рекламные формулировки."""
    low = text.lower()
    ...
    # Проверка на избыток восклицательных знаков
    if text.count('!') > 1:
        return True
    
    # Проверка на КАПС (больше 2 слов подряд)
    words = text.split()
...
# ============ ГЕНЕРАЦИЯ ТЕКСТА (СТРОГО НЕЙТРАЛЬНАЯ) ============
...
def build_dynamic_prompt(title: str, summary: str, style: dict, structure: str) -> str:
    ...
    structure_instructions = {
        "inverted_pyramid": """
Структура:
1. ЛИД — что произошло (1–2 предложения).
2. ДЕТАЛИ — как это работает, какие результаты (2–3 предложения).
3. КОНТЕКСТ — к чему это относится (1 предложение).
""",
        "straight_news": """
Структура:
1. ГЛАВНОЕ — что произошло (1 предложение).
2. ПОДРОБНОСТИ — ключевые факты (2–3 предложения).
3. ИТОГ — значение для отрасли (1 предложение).
"""
    }

    prompt = f"""
Ты — редактор новостной ленты. Перепиши новость в формате краткой заметки.
...
• 350–420 символов.
• Только русский язык.
• 0–1 эмодзи (только если уместно по контексту).
• Только факты из исходника. Ничего не додумывай.
• Пиши в третьем лице, от лица наблюдателя.

ЗАПРЕЩЕНО:
• Любые призывы: «попробуйте», «оцените», «не пропустите».
• Оценки: «революционный», «потрясающий», «лучший», «убийца».
• Восклицательные знаки (максимум один, и то нежелательно).
• Обращения к читателю: «вы», «мы», «друзья».
• Клише: «будущее наступило», «мир изменился», «новая эра».
• Вводные: «Итак», «Ну что», «Важная новость».
• Рекламный и продающий тон.

Выдай только текст поста. Без хештегов, ссылок, подписей.
"""
    return prompt


def short_summary(title: str, summary: str, link: str) -> Optional[str]:
    """Генерирует нейтральный новостной пост."""
    ...
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "Ты — новостной редактор. Пишешь сухо, по фактам, "
                        "без эмоций и без рекламного тона. Никаких призывов и оценок."
                    )
                },
                {"role": "user", "content": prompt}
            ],
            temperature=0.3,  # низкая температура для более сухого стиля
            max_tokens=500,
        )
        core = res.choices[0].message.content.strip()

        # Убираем кавычки если модель обернула текст
        if core.startswith('"') and core.endswith('"'):
            core = core[1:-1]
        if core.startswith('«') and core.endswith('»'):
            core = core[1:-1]

        # Проверка на рекламный тон
        ...
        return core + hashtag_line + source_line
